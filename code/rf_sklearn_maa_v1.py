...
os.chdir("/Users/mallen/Documents/q10/web_crop/code")
seed = 277
...
cbar.ax.set_yticklabels(['alfalfa', 
                         ...
                         'vine'])
plt.tight_layout()
plt.savefig("../plots/rf_sklearn_maa_outputclass_v1.png", dpi = 1000)

### output as image
# crs/trans from reference image
aviris = rio.open("../data/f170607t01p00r17_corr_v1g_img_clip_warp_sub.tif")
metao = aviris.meta.copy()
metao.update({'count': 1,
             'nodata': -999})

# change nans to 
featpred_arr_reshape = np.nan_to_num(featpred_arr_reshape)

# output
with rio.open("../data/predicted.tif", 'w', **metao) as dst:
    dst.write(featpred_arr_reshape[None,:,:].astype('float32'))
...
# save to csv
cm_accdf.to_csv("../data/rf_sklearn_maa_confusionmatrix_v1.csv")
